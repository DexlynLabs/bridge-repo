module hp_mailbox::mailbox {
    ...
    use hp_mailbox::events::{Self, ProcessEvent, DispatchEvent, InsertedIntoTree,OwnershipTransferStarted,OwnershipTransferred};
    ...
    const ERROR_DOMAIN_ALREADY_SET_OR_INVALID_DOMAIN: u64 = 6;
    ...
    struct MailBoxState has key, store {
        ...
        delivered: Table<vector<u8>, bool>,
        // event handlers
        ...
        transfer_initate_events: EventHandle<OwnershipTransferStarted>,
        tranfer_complete_events: EventHandle<OwnershipTransferred>,
    }
    ...
    fun init_module(account: &signer) {
        ...
        move_to<MailBoxState>(account, MailBoxState {
            ...
            delivered: table::new<vector<u8>, bool>(),
            // events
            ...
            transfer_initate_events: account::new_event_handle<OwnershipTransferStarted>(account),
            tranfer_complete_events: account::new_event_handle<OwnershipTransferred>(account),
        });
    }
    ...
    public entry fun initialize(
        ...
        assert!(state.local_domain == NONE_DOMAIN && domain != NONE_DOMAIN, error::already_exists(ERROR_DOMAIN_ALREADY_SET_OR_INVALID_DOMAIN));
    ...
    fun inbox_process(
        ...
        assert!(!table::contains(&state.delivered, id), ERROR_ALREADY_DELIVERED);

        // mark it as delivered
        table::add(&mut state.delivered, id, true);
    ...
    public fun transfer_ownership(account: &signer, new_owner_address: address) acquires MailBoxState {
        assert_owner_address(signer::address_of(account));
        let state = borrow_global_mut<MailBoxState>(@hp_mailbox);
        state.pending_owner_address = new_owner_address;
        event::emit_event<OwnershipTransferStarted>(
            &mut state.transfer_initate_events,
            events::new_initiate_transfer_event(
                state.owner_address,
                new_owner_address
            )
        );
    ...
    /// Accept ownership of MailBox
    public fun accept_ownership(account: &signer) acquires MailBoxState {
        let state = borrow_global_mut<MailBoxState>(@hp_mailbox);
        assert!(state.pending_owner_address == signer::address_of(account), ERROR_INVALID_OWNER);
        event::emit_event<OwnershipTransferred>(
            &mut state.tranfer_complete_events,
            events::new_transfer_event(
                state.owner_address,
                state.pending_owner_address
            )
        );
    ...
    public fun delivered(message_id: vector<u8>): bool acquires MailBoxState {
        let state = borrow_global<MailBoxState>(@hp_mailbox);
        table::contains(&state.delivered, message_id)
    }
