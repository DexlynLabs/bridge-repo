module hp_isms::multisig_ism {
  ...
  use hp_isms::events::{Self,OwnershipTransferred,OwnershipTransferStarted};
  ...
  const ERROR_VALIDATORS_NOT_UNIQUE: u64 = 3;
  ...
  fun init_module(account: &signer) {
    move_to<ISM>(account, ISM {
      ...
      pending_owner: @0x0,
      transfer_initate_events: new_event_handle<OwnershipTransferStarted>(account),
      tranfer_complete_events:new_event_handle<OwnershipTransferred>(account),
    });
  }
  ...
  public entry fun set_validators_and_threshold(
    ...
    let uniqueness_check:vector<address> = vector::empty<address>();
    vector::for_each_ref(&validators,|validator| {
      if(vector::contains(&uniqueness_check,validator)) {
        abort ERROR_VALIDATORS_NOT_UNIQUE;
      }
      else {
      vector::push_back(&mut uniqueness_check,*validator);
      }
    });
  ...
  /// Accept ownership of multisig_ism contract
  entry fun accept_ownership(
    account: &signer
  ) acquires ISM {
    let state = borrow_global_mut<ISM>(@hp_isms);
    assert!(state.pending_owner == signer::address_of(account), ERROR_INVALID_OWNER);
  ...
  public fun verify(
    ...
    let merkle_index = ism_metadata::merkle_index_u32(metadata);
