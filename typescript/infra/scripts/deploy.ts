...
async function main() {
  ...
  if (module === Modules.PROXY_FACTORY) {
    ...
    let warpRouteIdLocal = '';
    if (envConfig.environment != 'test') {
      if (!warpRouteId) {
        throw new Error('Warp route ID is required for WARP module');
      }
      warpRouteIdLocal = warpRouteId;
    } else {
      warpRouteIdLocal = 'USDC/test1-ethereum';
    }
    const ismFactory = HyperlaneIsmFactory.fromAddressesMap(
      getAddresses(envConfig.environment, Modules.PROXY_FACTORY),
      multiProvider,
    );
    config = await getWarpConfig(multiProvider, envConfig, warpRouteIdLocal);
    // console.log(config);
