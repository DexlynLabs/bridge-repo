...
const APTOS_VALIDATOR_KEYS: &[&str] = &[
    // aptoslocalnet1,
...
fn main() -> ExitCode {
    ...
    // TODO: may be need to add aptos as well
    let mut validator_origin_chains = ["test1", "aptoslocalnet1"].to_vec();
    let mut validator_keys = ETH_VALIDATOR_KEYS.to_vec();
    let mut aptos_validator_keys = APTOS_VALIDATOR_KEYS.to_vec();
    validator_keys.append(&mut aptos_validator_keys);
    let mut validator_count: usize = validator_keys.len() + aptos_validator_keys.len();
    ...
    let relayer_env = if config.sealevel_enabled {
        ...
        relayer_env.arg("relayChains", "test1,aptoslocalnet1")
    };
    ...
    let base_validator_env = common_agent_env
        ...
        .hyp_env("CHAINS_APTOSLOCALNET2_RPCCONSENSUSTYPE", "httpFallback")
        .hyp_env("CHAINS_APTOSLOCALNET1_SIGNER_KEY", APTOS_VALIDATOR_KEYS[0])
